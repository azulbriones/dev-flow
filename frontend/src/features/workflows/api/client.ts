...
export async function getWorkflows(): Promise<Workflow[]> {
  return fetchJson<Workflow[]>(API_ENDPOINTS.workflows);
}

export async function getWorkflow(id: number): Promise<Workflow> {
  return fetchJson<Workflow>(`${API_ENDPOINTS.workflows}/${id}`);
}
...
export async function getExecutions(): Promise<Execution[]> {
  return fetchJson<Execution[]>(API_ENDPOINTS.executions);
}
